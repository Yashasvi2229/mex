 ...
 *
 * An entity id is `mx_` plus a 26-character Crockford Base32 ULID. The single
 * invariant that matters is that **an id is never derived from a title or a
 * path**, and is never regenerated because a heading was renamed or a file was
 * moved. Everything downstream — relations, backlinks, topic membership,
 * grounding — refers to entities by id precisely so those edits are free.
 */
 ...
 */
export const ENTITY_ID_PATTERN = /^mx_[0-9A-HJKMNP-TV-Z]{26}$/;
...
export function isEntityId(value: unknown): value is EntityId {
  if (typeof value !== "string" || value.length !== ENTITY_ID_LENGTH) return false;
  if (!value.startsWith(ENTITY_ID_PREFIX)) return false;
  return isUlid(value.slice(ENTITY_ID_PREFIX.length));
}
...
export function normalizeEntityId(value: unknown): EntityId | null {
  ...
  if (trimmed.length !== ENTITY_ID_LENGTH) return null;
  const candidate = ENTITY_ID_PREFIX + trimmed.slice(ENTITY_ID_PREFIX.length).toUpperCase();
  if (trimmed.slice(0, ENTITY_ID_PREFIX.length).toLowerCase() !== ENTITY_ID_PREFIX) return null;
...
export function entityIdTimestamp(id: string): number | null {
  ...
  return normalized === null ? null : decodeUlidTime(normalized.slice(ENTITY_ID_PREFIX.length));
}
...
/** Deterministic id ordering. Ids are ULIDs, so this is also creation order. */

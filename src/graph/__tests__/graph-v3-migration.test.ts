 ...
 *
 * **The migration being real.** `openReadOnlyGraphDatabase` demanded exact
 * version equality, so shipping a version bump without a migration forces every
 * existing user to rebuild a graph whose nodes and edges did not change by one
 * byte. A rebuild prompt is not a migration, and `requires no rebuild` below is
 * the assertion that says so.
 *
 * **The read floor.** Reads tolerate a v2 database rather than throwing at it,
 * because read-only opens cannot migrate and they are what `mex graph
 * query|scope|get` use. The tolerance is bounded, and v1 stays out.
 */
...
describe("migrateV2ToV3", () => {
  it("migrates a v2 database in place and requires no rebuild", () => {
    ...
    try {
      expect(readSchemaVersion(db)).toBe(3);
      // The point of the whole exercise: a v2 user upgrades and keeps their
      // graph. A rebuild prompt here would mean re-extracting every file.
...
describe("the read-only schema floor", () => {
  it("states the policy directly", () => {
    expect(MIN_READABLE_SCHEMA_VERSION).toBe(2);
    expect(isReadableSchemaVersion(DB_SCHEMA_VERSION)).toBe(true);
    expect(isReadableSchemaVersion(MIN_READABLE_SCHEMA_VERSION)).toBe(true);
    // v1's identity and resolution facts are untrustworthy under v2 — the v1
    // rung marks the graph rebuild-required — so reads must never serve one.
    expect(isReadableSchemaVersion(1)).toBe(false);
    expect(isReadableSchemaVersion(null)).toBe(false);
    // Newer than this build understands is still a hard stop.
    expect(isReadableSchemaVersion(DB_SCHEMA_VERSION + 1)).toBe(false);
  });

  it("serves a v2 database read-only, without migrating it", () => {
    // The regression this exists to prevent: before the floor, a user's first
    // `mex graph query` after upgrading threw, because the read-only open
    // demanded exact equality and could not migrate.
    ...
    try {
      ...
      // Still v2 afterwards: a read command must not write to the user's graph.
      expect(rows(db, "SELECT scaffold_file FROM _mex_grounded_source ORDER BY scaffold_file")).toHaveLength(3);
    } finally {
      db.close();
    }

...
/**
 * The oldest schema a **read-only** open will serve without migrating.
 *
 * Read-only opens cannot migrate — that is the point of them — and they are on
 * the hot path: `mex graph query|scope|get` all open read-only
 * (`cli-agent.ts`, `openSession`). Before this floor existed the open demanded
 * *exact* version equality, so bumping DB_SCHEMA_VERSION made a user's first
 * retrieval command after an upgrade throw `GraphRebuildRequiredError` before
 * any writable open got a chance to migrate.
 *
 * The alternative — have the CLI attempt one writable migrating open and fall
 * back — fails precisely where read-only mode exists to work (a sandbox or a
 * read-only filesystem; see the immutable-open fallback below), and it turns a
 * read command into a write to the user's graph. So reads tolerate an older
 * schema instead.
 *
 * **The tolerance is bounded on purpose, and the bound is a judgement about
 * facts, not about convenience.** v2 to v3 changes only the grounding
 * baseline's key; nodes, edges, fingerprints, aliases and source chunks are
 * bit-identical, so a v2 database answers a retrieval query correctly. v1 is
 * *not* readable and must never become so: its identity and resolution facts
 * are untrustworthy under v2, which is why {@link migrateV1ToV2} marks the
 * graph rebuild-required. A future migration that invalidates read facts the
 * same way raises this floor, and the tolerance ends there.
 */
export const MIN_READABLE_SCHEMA_VERSION = 2;
...
export function openGraphDatabase(
  ...
  if (!hasVersions) {
    ...
    if (current < DB_SCHEMA_VERSION) migrate(db, schema, current);
    else db.exec(schema);
  }
...
function openReadOnlyGraphDatabase(dbPath: string): SqliteDatabase {
  const validate = (db: SqliteDatabase): SqliteDatabase => {
    ...
    if (!tableExists(db, "schema_versions") || !isReadableSchemaVersion(readSchemaVersion(db))) {
      throw new GraphRebuildRequiredError();
    }
...
/**
 * Whether a read-only open may serve this database.
 *
 * Exported so the policy is assertable directly rather than only through a
 * thrown error, which is the difference between a test that pins the rule and
 * a test that pins one symptom of it.
 */
export function isReadableSchemaVersion(version: number | null): boolean {
  return version !== null
    && version >= MIN_READABLE_SCHEMA_VERSION
 ...
 * store, never two).
 *
 * SQLite cannot alter a primary key, so this is a table rebuild. It is a real
 * migration and deliberately **not** a rebuild prompt: `openReadOnlyGraphDatabase`
 * used to demand exact version equality, so shipping the bump without this
 * would force every existing user to rebuild a graph whose nodes and edges did
